...
switch ($action) {
    ...
    case "sslagree":
        ...
        $redirectTo = isset($_SESSION["return"])
            ? $_SESSION["return"]
            : "index.php";
    ...
    case "authenticate":
        ...
        $credentials =
            (!empty($_POST["user"]) ? clean($_POST["user"]) : "") .
            ":" .
    ...
    case "clear-cookies":
        ...
        if ($CONFIG["cookies_on_server"]) {
            ...
            if (
                is_writable(
                    $file = $CONFIG["cookies_folder"] . glype_session_id()
                )
            ) {
                # Delete it
                unlink($file);
    ...
    case "cookies":
        ...
        if ($CONFIG["cookies_on_server"]) {
            ...
            if (
                file_exists(
                    $cookieFile = $CONFIG["cookies_folder"] . glype_session_id()
                ) &&
    ...
    case "edit-browser":
        ...
        $browser["user_agent"] = isset($_POST["user-agent"])
            ? clean($_POST["user-agent"])
            : "";
        $browser["referrer"] = empty($_POST["real-referrer"])
            ? "custom"
            : "real";

        if ($browser["referrer"] == "custom") {
            $browser["referrer"] = isset($_POST["custom-referrer"])
                ? clean($_POST["custom-referrer"])
                : "";
        }
