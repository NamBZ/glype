...
define(
    ...
    "http" .
        ...
        $_SERVER["HTTP_HOST"] .
        preg_replace(
            "#/(?:(?:includes/)?[^/]*|" . preg_quote(SCRIPT_NAME) . '.*)$#',
            "",
            $_SERVER["PHP_SELF"]
        )
);
...
$phrases["invalid_url"] =
    "The requested URL was not recognised as a valid URL. Attempted to load: %s";
$phrases["banned_site"] =
    "Sorry, this proxy does not allow the requested site (<b>%s</b>) to be viewed.";
$phrases["file_too_large"] =
    "The requested file is too large. The maximum permitted filesize is %s MB.";
$phrases["server_busy"] =
    "The server is currently busy and unable to process your request. Please try again in a few minutes. We apologise for any inconvenience.";
$phrases["http_error"] =
    'The requested resource could not be loaded because the server returned an error:<br> &nbsp; <b>%s %s</b> (<span class="tooltip" onmouseout="exit()" onmouseover="tooltip(\'%s\');">?</span>).';
$phrases["curl_error"] =
    "The requested resource could not be loaded. libcurl returned the error:<br><b>%s</b>";
$phrases["unknown_error"] =
    "The script encountered an unknown error. Error id: <b>%s</b>.";
...
$httpErrors = [
    "404" => "A 404 error occurs when the requested resource does not exist.",
];
...
if (
    !defined("MULTIGLYPE") &&
    file_exists(
        $tmp = GLYPE_ROOT . "/themes/" . $CONFIG["theme"] . "/config.php"
    )
) {
    # Load it
    include $tmp;
    ...
    if (!$CONFIG["enable_blockscript"]) {
        ...
        foreach ($CONFIG["ip_bans"] as $ip) {
            ...
            if ($ip[$pos] == "/") {
                ...
                if (
                    substr(decbin(ip2long($net)), 0, $mask) ===
                    substr($ownBin, 0, $mask)
                ) {
...
if (
    $CONFIG["path_info_urls"] &&
    !empty($_SERVER["PATH_INFO"]) &&
    preg_match(
        '#/b([0-9]{1,5})(?:/f([a-z]{1,10}))?/?$#',
        $_SERVER["PATH_INFO"],
        $tmp
    )
) {
...
} elseif (!empty($_SESSION["bitfield"])) {
...
function proxyURL($url, $givenFlag = false)
{
    ...
    if ($CONFIG["path_info_urls"] && $options["encodeURL"]) {
        return GLYPE_BROWSE .
            "/" .
            str_replace("%", "_", chunk_split($url, 8, "/")) .
            "b" .
            $bitfield .
            "/" .
            ($addFlag ? "f" . $addFlag : "") .
...
function absoluteURL($input)
{
    ...
    if (strpos($input, "../")) {
        ...
        $oldPath = $path = parse_url($input, PHP_URL_PATH);
...
function getTemplatePath($file)
{
    ...
    if (
        ...
        if (
            ...
            !file_exists(
                $return = GLYPE_ROOT . "/themes/default/" . $file . ".php"
            )
        ) {
...
function replaceContent($content)
{
    ...
    return replaceThemeTags(
        preg_replace(
            "#<!-- CONTENT START -->.*<!-- CONTENT END -->#s",
            $content,
            $output
        )
    );
...
function injectionJS()
{
    ...
    if ($options["encodePage"]) {
        $fullURL = isset($URL["href"])
            ? arcfour("encrypt", $GLOBALS["unique_salt"], $URL["href"])
            : "";
        $targetHost = isset($URL["scheme_host"])
            ? arcfour("encrypt", $GLOBALS["unique_salt"], $URL["scheme_host"])
            : "";
        $targetPath = isset($URL["path"])
            ? arcfour("encrypt", $GLOBALS["unique_salt"], $URL["path"])
            : "";
    } else {
    ...
    $optional =
        isset($URL) && $CONFIG["override_javascript"] ? ",override:1" : "";
...
function error($type, $allowReload = false)
{
    ...
    if ($args) {
        ...
        $errorText = call_user_func_array(
            "sprintf",
            array_merge((array) $phrases[$type], $args)
        );
    } else {
    ...
    if (strlen($return) > 0) {
        $themeReplace["error"] .=
            ...
            htmlentities($return) .
            '">Reload ' .
            htmlentities(deproxyURL($return)) .
            "</a>]</p>";
    }
...
function currentURL()
{
    ...
    return GLYPE_BROWSE .
        $separator .
        (isset($_SERVER[$method]) ? $_SERVER[$method] : "");
}
...
function checkTmpDir($path, $htaccess = false)
{
    ...
    if (file_exists($path)) {
        ...
        # inside the temp dir)
        if (
            is_writable($CONFIG["tmp_dir"]) &&
            realpath($CONFIG["tmp_dir"]) == realpath(dirname($path) . "/") &&
...
function glype_session_id()
{
    ...
    if ($session_id == "") {
        return "";
    } elseif (!preg_match('/^[a-zA-Z0-9-]+$/', $session_id)) {
