...
class parser
{
    ...
    function HTMLDocument($input, $insert = "", $inject = false, $footer = "")
    {
        ...
        global $charset;
        if (!isset($charset)) {
            $meta_equiv = preg_match(
                "#(<meta[^>]*http\-equiv\s*=[^>]*>)#is",
                $input,
                $tmp,
                PREG_OFFSET_CAPTURE
            )
        ...
        $input = preg_replace(
            "#RTCPeerConnection#s",
            "disabled___RTCPeerConnection",
            $input
        );

        # Remove titles if option is enabled
        if (
            $this->htmlOptions["stripTitle"] ||
            $this->htmlOptions["encodePage"]
        ) {
            $input = preg_replace("#<title.*?</title>#is", "", $input, 1);
            $input = preg_replace(
                '#<meta[^>]*name=["\'](title|description|keywords)["\'][^>]*>#is',
                "",
                $input,
                3
            );
            $input = preg_replace(
                '#<link[^>]*rel=["\'](icon|shortcut icon)["\'][^>]*>#is',
                "",
                $input,
                2
            );
        ...
        $input = preg_replace_callback(
            '#<base href\s*=\s*([\\\'"])?((?(1)(?(?<=")[^"]{1,2048}|[^\\\']{1,2048})|[^\s"\\\'>]{1,2048}))(?(1)\\1|)[^>]*>#i',
            "html_stripBase",
            $input,
            1
        );
        ...
        $input = preg_replace_callback(
            '#content\s*=\s*(["\\\'])?[0-9]+\s*;\s*url=([\\\'"]|&\#39;)?((?(?<=")[^"]+|(?(?<=\\\')[^\\\']+|[^\\\'" >]+)))(?(2)\\2|)(?(1)\\1|)#i',
            "html_metaRefresh",
            $input,
            1
        );
        ...
        while (
            ...
            $new .= $this->$parseFunction(
                substr($input, $innerStart, $innerEnd - $innerStart)
            );
        ...
        if ($insert !== false) {
            ...
            if (($useFrames = stripos($input, "<frameset")) !== false) {
                ...
                $input = preg_replace_callback(
                    '#<frame[^>]+src\s*=\s*([\\\'"])?((?(1)(?(?<=")[^"]{1,2048}|[^\\\']{1,2048})|[^\s"\\\'>]{1,2048}))(?(1)\\1|)#i',
                    "html_flagFrames",
                    $input
                );
            }
    ...
    function HTML($input)
    {
        ...
        if ($this->htmlOptions["stripObjects"]) {
            ...
            $input = preg_replace_callback(
                '#<param[^>]+value\s*=\s*([\\\'"])?((?(1)(?(?<=")[^"]{1,2048}|[^\\\']{1,2048})|[^\s"\\\'>]{1,2048}))(?(1)\\1|)[^>]*>#i',
                "html_paramValue",
                $input
            );
        ...
        # Parse onX events
        $input = preg_replace_callback(
            '#\b(on(?<!\.on)[a-z]{2,20})\s*=\s*([\\\'"])?((?(2)(?(?<=")[^"]{1,2048}|[^\\\']{1,2048})|[^\s"\\\'>]{1,2048}))(?(2)\\2|)#i',
            [&$this, "html_eventJS"],
            $input
        );
        ...
        $input = preg_replace_callback(
            '#style\s*=\s*([\\\'"])?((?(1)(?(?<=")[^"]{1,2048}|[^\\\']{1,2048})|[^\s"\\\'>]{1,2048}))(?(1)\\1|)#i',
            [&$this, "html_elementCSS"],
            $input
        );
    ...
    {
        return $this->htmlOptions["stripJS"]
            ? ""
            : $input[1] . "=" . $input[2] . $this->JS($input[3]) . $input[2];
    }
    ...
    function JS($input)
    {
        ...
        $assignmentPattern = ["before" => ".", "after" => "="];
        $methodPattern = ["before" => ".", "after" => "("];
        $functionPattern = ["after" => "("];

        # Configure strings to search for, starting with always replaced commands
        $search["innerHTML"][] = $assignmentPattern;
        $search["location"][] = ["after" => ".", "id" => "replace()"];
        ...
        if (in_array("watch", $flags)) {
            $search["location"][] = ["after" => "=", "id" => "assignment"];
        }
        ...
        while ($offset < $length) {
            ...
            switch ($valid) {
                ...
                case "innerHTML":
                    ...
                    $replacement = sprintf(
                        "parse%s(%s)",
                        $command == "innerHTML" ? "HTML" : "URL",
                        substr($input, $postCharPos, $valueLength)
                    );
                ...
                case "eval":
                    ...
                    if (
                        isset($input[$commandPos - 1]) &&
                        strpos(
                            "abcdefghijklmnopqrstuvwxyz123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ_",
                            $input[$commandPos - 1]
                        ) !== false
                    ) {
                        break;
...
function html_form($input)
{
    ...
    $input[1] = preg_replace_callback(
        '#\baction\s*=\s*([\\\'"])?((?(1)(?(?<=")[^"]{1,2048}|[^\\\']{1,2048})|[^\s"\\\'>]{1,2048}))(?(1)\\1|)#i',
        "html_formAction",
        $input[1]
    );
    ...
    if (stripos($input[1], "multipart/form-data")) {
        $input[2] = preg_replace_callback(
            '#name\s*=\s*([\\\'"])?((?(1)(?(?<=")[^"]{1,2048}|[^\\\']{1,2048})|[^\s"\\\'>]{1,2048}))(?(1)\\1|)#i',
            "html_inputName",
            $input[2]
        );
    }
...
function html_attribute($input)
{
    ...
    return str_replace(
        $input[3],
        htmlspecialchars(proxyURL(htmlspecialchars_decode($input[3]), $flag)),
        $input[0]
    );
}
...
function analyze_js($input, $start, $argPos = false)
{
    ...
    for (
        ...
        $length = strlen($input),
        $end = false,
        $openObjects = $openBrackets = $openArrays = 0;
        $end === false && $i < $length;
        ++$i
    ) {
        $char = $input[$i];
...
function encodeBlock($input)
{
    ...
    return '<script type="text/javascript">document.write(arcfour(ginf.enc.u,base64_decode(\'' .
        arcfour("encrypt", $GLOBALS["unique_salt"], $input) .
        '\')));</script>';
}
